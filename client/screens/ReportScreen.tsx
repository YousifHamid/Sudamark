...
export default function ReportScreen() {
    ...
    const handleSubmit = async () => {
        if (!username || !complaintType || !details) {
            Alert.alert(t("error"), isRTL ? "يرجى ملء جميع الحقول المطلوبة" : "Please fill all required fields");
            return;
        }
        ...
        <ThemedView style={styles.container}>
            ...
            >
                <View style={styles.header}>
                    ...
                    <ThemedText type="h2" style={{ marginTop: Spacing.md }}>{isRTL ? "تقديم شكوى" : "Report Issue"}</ThemedText>
                ...
                <View style={styles.form}>
                    <ThemedText type="small" style={[styles.label, { color: theme.textSecondary }, isRTL && styles.rtlText]}>
                        {isRTL ? "اسم المستخدم المبلغ عنه" : "Reported Username / ID"} *
                    </ThemedText>
                    <TextInput
                    ...
                    <Pressable
                        style={[styles.input, { backgroundColor: theme.backgroundSecondary, borderColor: theme.border, justifyContent: "center" }]}
                        onPress={() => setShowTypePicker(true)}
                    >
                        <ThemedText style={{ color: complaintType ? theme.text : theme.textSecondary, textAlign: isRTL ? "right" : "left" }}>
                            {complaintType
                                ? (isRTL
                    ...
                    <Button onPress={handleSubmit} disabled={isSubmitting} style={{ marginTop: Spacing.xl }}>
                        {isSubmitting ? (isRTL ? "جاري الإرسال..." : "Submitting...") : (isRTL ? "إرسال الشكوى" : "Submit Complaint")}
                    </Button>
                </View>
                        ...
                        <View style={[styles.modalContent, { backgroundColor: theme.backgroundDefault }]}>
                            {complaintTypes.map((type) => (
                                ...
                                >
                                    ...
                                    {complaintType === type.id && <Feather name="check" size={18} color={theme.primary} />}
                                </Pressable>
                            ))}
                        </View>
                    </Pressable>
